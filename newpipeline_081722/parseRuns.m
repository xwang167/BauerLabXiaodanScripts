...
% Eliminate bad runs
    ...
    if sum(~cellfun(@isempty,strfind(excelData.Properties.VariableNames,'BadRuns'))) > 0
        ...
        if ~isnan(badRuns) %% xw 220718 only display there is bad run
...
%Are there bad presentations?
    ...
    for run = uniqueRuns
        ...
        if isempty(darkFramesInd)
            ...
            runsInfo(totalRunInd).totNum_frame=runTime*samplingRate*numCh+(numCh*darkFramesInd(end)); %%add no darkFrame
        end
        ...
        if isempty(darkFramesInd)
            runsInfo(totalRunInd).totNum_frame=runTime*samplingRate*numCh;%%xw 220730 add no darkframe condition
        else
