...
%Some user input!
excelFile="X:\CodeModification\CodeModification.xlsx";
excelRows=13;
        ...
        if ~exist(runInfo.saveMaskFile,'file') || (~ismember('xform_StimROIMask', variableInfo) && runInfo.session =="stim") %%For laser case, we have a landmarksandMask without stimMask beofre the processing script
            ...
            if runInfo.session =="stim"
                ...
                for jj=1:runInfo.numCh
                    ...
                    title(['CH:', num2str(jj)])
                    if jj==runInfo.rgbInd(2) %% xw 220711 the green reflectance is not always the 2 channel
                ...
                disp(runInfo.mouseName)% xw 220806 to see which side for whisker
            ...
            elseif ~exist(runInfo.saveMaskFile,'file')
                ...
                xform_StimROIMask=affineTransform(StimROIMask,I); %xw 220806 xform
                xform_StimROIMask=single(uint8(xform_StimROIMask));
        ...
        if ~exist(runInfo.saveMaskFile,'file') || (~ismember('xform_StimROIMask', variableInfo) && runInfo.session =="stim")%%xw 220603 for laser case, we have a landmarksandMask without stimMask beofre the processing script
            ...
            if runInfo.session =="stim"
                ...
                for jj=1:runInfo.numCh
                    ...
                    imagesc(squeeze(tmp(:,:,jj)))
                    title(['CH:', num2str(jj)])
                    if ~isempty(runInfo.laserChInd)
                        ROI_channel = runInfo.laserChInd;%xw 220720 if there is laser frame, ROI from laser frame
                    else
                        ROI_channel = runInfo.rgbInd(2);% xw 2207011 the green reflectance is not always the 2 channel
                    end
                    if jj== ROI_channel
                        title(['PICK ME! CH:', num2str(jj)])
                    end
...
for runInd = 1:runNum
    ...
    elseif ((exist(stimExist) && exist(stimExist2) && strcmp(runInfo.session,'stim') ) ||  (exist(fcExist) && strcmp(runInfo.session,'fc') ))
        ...
        if ~contains(runInfo.system,'EastOIS2')
            ...
            raw=FindandFixDroppedFrames(raw,runInfo); %JPC added %%xw 220730 wrong
    ...
    if runInfo.session =="fc"
        ...
        for contrast=1:numel(runInfo.Contrasts)
            [whole_spectra_map(:,:,:,contrast),avg_cort_spec(:,contrast),powerMap(:,:,:,contrast),hz, global_sig_for(:,contrast),glob_sig_power(:,contrast)]= PowerAnalysis(squeeze(data_full(:,:,:,contrast)),runInfo.samplingRate,xform_isbrain,false);%xw 220718
        end
        ...
        for ii=1:size(data_full,4)
            ...
            data_isa(:,:,:,ii)=resample(squeeze(data_isa_tmp(:,:,:,ii)), (2*fRange_ISA(2)*1.25)*5 ,runInfo.samplingRate*5,'Dimension',3); %xw 220718
            
            data_delta_tmp(:,:,:,ii)=filterData(data_full(:,:,:,ii),fRange_delta(1),fRange_delta(2),runInfo.samplingRate,xform_isbrain);
            data_delta(:,:,:,ii)=resample(squeeze(data_delta_tmp(:,:,:,ii)), (2*fRange_delta(2)*1.25)*5 ,runInfo.samplingRate*5,'Dimension',3);%xw 220718
        end
        ...
        %Delta Band
        if ~isempty(runInfo.fluorChInd) || ~isempty(runInfo.FADChInd)
            ...
            elseif ~isempty(runInfo.FADChInd) && isempty(runInfo.fluorChInd)
                ContFig2Save= {'FAD'};
            end
            clear fh
            [fh, seedFC, seedFCMap,seedCenter,seedRadius,bilatFCMap]...
    ...
    elseif runInfo.session =="stim"
        ...
        hz_size= 2^floor(log2(size(data_full,3)))/4+1; %xw 07/26/2022 change to the same as fc
        whole_spectra_map=nan(128,128,hz_size,numel(runInfo.Contrasts));
        avg_cort_spec=nan(hz_size,numel(runInfo.Contrasts));
        powerMap=nan(128,128,3,numel(runInfo.Contrasts));
        global_sig_for=nan(hz_size,numel(runInfo.Contrasts));
        ...
        for contrast=1:numel(runInfo.Contrasts)
            [whole_spectra_map(:,:,:,contrast),avg_cort_spec(:,contrast),powerMap(:,:,:,contrast),hz, global_sig_for(:,contrast),glob_sig_power(:,contrast)]= PowerAnalysis(squeeze(data_full(:,:,:,contrast)),runInfo.samplingRate,xform_isbrain);%% 220731 delete fft_bool
        end
        ...
        runInfo.reSamplingRate=(2*fRange_delta(2)*1.25); %change the sampling rate to fit the resample frequency
