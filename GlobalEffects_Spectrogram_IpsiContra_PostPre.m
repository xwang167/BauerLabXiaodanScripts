...
for freqInd = 1:4
    ...
    for runInd=mouse_ind
        ...
        fc_pre_ipsi = median(squeeze(median(abs(S1_fc_ipsi(freqBand{freqInd}(1):freqBand{freqInd}(2),:)))));
        stim_pre_ipsi = median(squeeze(median(abs(S1_stim_ipsi(freqBand{freqInd}(1):freqBand{freqInd}(2),:)))));
        if mouseInd == 1
        ...
        subplot(211)
        plot(T2/60,mean(abs(S2_fc_ipsi(freqBand{freqInd}(1):freqBand{freqInd}(2),:)))/fc_pre_ipsi,'-ko',...
        ...
        subplot(212)
        plot(T2/60,mean(abs(S2_stim_ipsi(freqBand{freqInd}(1):freqBand{freqInd}(2),:)))/stim_pre_ipsi,'-ko',...
        ...
        fc_pre_bilat = median(squeeze(median(abs(S1_fc_bilat(freqBand{freqInd}(1):freqBand{freqInd}(2),:)))));
        stim_pre_bilat = median(squeeze(median(abs(S1_stim_bilat(freqBand{freqInd}(1):freqBand{freqInd}(2),:)))));

        subplot(211)
        plot(T2/60,mean(abs(S2_fc_bilat(freqBand{freqInd}(1):freqBand{freqInd}(2),:)))/fc_pre_bilat,'-ko',...
            'MarkerSize',10, 'MarkerEdgeColor',colorFc{mouseInd},'MarkerFaceColor',colorFc{mouseInd})
        hold on
        yline(1,'-b')
        hold on
        ...
        title('Without PS')
        subplot(212)
        plot(T2/60,mean(abs(S2_stim_bilat(freqBand{freqInd}(1):freqBand{freqInd}(2),:)))/stim_pre_bilat,'-ko',...
            'MarkerSize',10, 'MarkerEdgeColor',colorStim{mouseInd},'MarkerFaceColor',colorStim{mouseInd})
        hold on
        yline(1,'-b')
        hold on
    ...
    figure(1)
    saveas(gca,strcat('X:\Paper2\GlobalEffects\30sPS\GroupResults\30s-Ipsi-spectrogram-PostPre-FCandStim-',startFreq,'-',endFreq,'.fig'))
    saveas(gca,strcat('X:\Paper2\GlobalEffects\30sPS\GroupResults\30s-Ipsi-spectrogram-PostPre-FCandStim-',startFreq,'-',endFreq,'.png'))

    figure(2)
    saveas(gca,strcat('X:\Paper2\GlobalEffects\30sPS\GroupResults\30s-Contra-spectrogram-PostPre-FCandStim-',startFreq,'-',endFreq,'.fig'))
    saveas(gca,strcat('X:\Paper2\GlobalEffects\30sPS\GroupResults\30s-Contra-spectrogram-PostPre-FCandStim-',startFreq,'-',endFreq,'.png'))
    close all
end
