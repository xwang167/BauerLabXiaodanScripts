...
for ii = 1:160
    if seedmapLocsOG(1,ii) ~= 0 && seedmapLocsOG(2,ii) ~= 0
        ...
        BilatInhibition(jj) = nanmedian(inhibitionMap_Calcium_NoGSR_mice(ROI,ii));
        jj = jj+1;
    end
end
...
for i=1:length(seedmapLocs)
    if seedmapLocs(1,i) ~= 0 && seedmapLocs(2,i) ~= 0
        brainMap(seedmapLocs(1,i)-pitch:seedmapLocs(1,i)+pitch,seedmapLocs(2,i)-pitch:seedmapLocs(2,i)+pitch) = BilatInhibition(jj);
        jj = jj + 1;
    end
end
...
for i=1:length(seedmapLocs)
    if seedmapLocs(1,i) ~= 0 && seedmapLocs(2,i) ~= 0
        brainMapOG(seedmapLocs(1,i)-pitch:seedmapLocs(1,i)+pitch,seedmapLocs(2,i)-pitch:seedmapLocs(2,i)+pitch) = BilatInhibition(jj);
        jj = jj + 1;
    end
end
imagesc(brainMapOG)
