function [fhandle]=visualize_fc_avg_matrix(Rs,Rsstd,mice,q,oi)
% plotting function for avg FC matrices

% IN: 
%   Rs: avg seeds x seeds x contrast x group across mice
%   Rsstd: std seeds x seeds x contrast x group across mice
...
%   fhandle: figure handle for saving
    ...
    % plot data
    for i=1:size(Rs,4) %loop through groups
        figure(i)  
        fhandle(i)=gcf;
        ha=tight_subplot(length(oi.con_num),2,[0.1 0.03],[0.1 0.1],[0.1 0.1]);  
        
        for d=1:length(oi.con_num) %loop through contrasts

            %plot avg values
            axes(ha(1+2*(d-1)));
            imagesc(Rs(:,:,d,i),[-1 1]); colormap jet;
            hold on;
            title(['contrast ' num2str(oi.con_num(d))])

            %plot std values
            axes(ha(2+2*(d-1)));
            imagesc(Rsstd(:,:,d,i),[-1 1]); colormap jet;
            hold on;
