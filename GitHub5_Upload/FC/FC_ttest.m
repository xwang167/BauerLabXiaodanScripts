function [tmap,p1,h1,h1_cc_rs]=FC_ttest(R_ms_group,group_index,isbrain2_aff,thresh,mice,oi)
% script to perform pixel-wise ttest on FC maps, uses cluster extent
...
%       seed. oi.nVx oi.nVy seeds contrast
    ...
    % only set regions outside FOV to 0
            ...
            for h=1:size(R_ms_group{1},3)
                ...
                elseif length(group_index)==1 %if 2 groups
                    ...
                    if strcmp(mice(1).test, '2-sample')
                        for w=1:length(oi.con_num) %loop through contrasts
                            if nansum(~isnan(R_ms_group{1}(f,g,h,w,:)) & R_ms_group{1}(f,g,h,w,:)~=0)<2 || nansum(~isnan(R_ms_group{2}(f,g,h,w,:)) & R_ms_group{2}(f,g,h,w,:)~=0)<2
                                h1(f,g,h,w)=NaN;
                                p1(f,g,h,w)=NaN;
                                tmap(f,g,h,w)=NaN;
                            else
    ...
    h1_cc=zeros(size(R_ms_group{1},1)*size(R_ms_group{1},2),size(h1,3),length(oi.con_num));
