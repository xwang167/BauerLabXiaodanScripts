function [rawdata_f,fulldetrend,all_contrasts,WL]=Proc1_sys_dep_LEDShifting(rawdata,dark,mice,oi,LEDcount)
...
%   WL: mouse specific WL
    ...
    %this is where it rotates which channel the LED is coming from
    %depending on the order
    if (LEDcount==1)
        ch1=1; ch2=2; ch3=3; ch4=4;
    end
    if (LEDcount==2)
        ch1=2; ch2=3; ch3=4; ch4=1;
    end
    if (LEDcount==3)
        ch1=3; ch2=4; ch3=1; ch4=2;
    end
    if (LEDcount==0)
        ch1=4; ch2=1; ch3=2; ch4=3;
    end
    rawdataShuffle=cat(3, rawdata_f(:,:,ch1,:),rawdata_f(:,:,ch2,:),rawdata_f(:,:,ch3,:),rawdata_f(:,:,ch4,:));
    rawdata_f=rawdataShuffle; clear rawdataShuffle;
