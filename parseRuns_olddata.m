function runsInfo = parseRuns_olddata(excelFile,excelRows)
...
%         runNumEnd(runNumEnd < runNumStart) = [];
    ...
    for run = uniqueRuns
        ...
        runNumStart = strfind(runFiles(1).name,'-cam1');
        saveFileName = runFiles(1).name; saveFileName = saveFileName(1:runNumStart-1);%%xw rawdata different format
        ...
        runsInfo(totalRunInd).totNum_frame=runTime*samplingRate*numCh+(numCh*darkFramesInd(end)); %jpc added
