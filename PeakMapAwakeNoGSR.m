...
for excelRow = excelRows
    ...
    for n = runs
        ...
        peakMap_HbO = squeeze(mean(xform_datahb(:,:,1,:,goodBlocks),5));
        peakMap_HbR = squeeze(mean(xform_datahb(:,:,2,:,goodBlocks),5));
        clear xform_datahb
        peakMap_HbT = peakMap_HbO + peakMap_HbR;
        peakMap_FAD = mean(xform_FADCorr(:,:,:,goodBlocks),4);
        clear xform_FADCorr
        peakMap_RawFAD = mean(xform_FAD(:,:,:,goodBlocks),4);
        clear xform_FAD
        peakMap_calcium = mean(xform_jrgeco1aCorr(:,:,:,goodBlocks),4);
        clear xform_jrgeco1aCorr
          peakMap_Rawcalcium = mean(xform_jrgeco1a(:,:,:,goodBlocks),4);
