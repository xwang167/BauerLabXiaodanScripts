function   [goodBlocks,ROI] = QC_stim_WT_FAD_camera2(oxy,deoxy,greenFluor,greenFluorCorr,green,jrgeco1a,jrgeco1aCorr,red,xform_isbrain,numBlock,numDesample,stimStartTime,stimduration,stimFreq,framerate,stimblocksize,stimbaseline,texttitle,output,input_ROI)
...
if ~isempty(goodBlocks)
    ...
    if ~isempty(jrgeco1a)
        ...
        peakMap_ROI= mean(greenFluorCorr_downsampled_blocks(:,:,stimStartTime+1:stimEndTime),3);
    ...
    if ~isempty(jrgeco1a)&&~isempty(greenFluor)
        ...
        legend('FADCorr cam2','FADCorr','HbO','HbR','Total','location','southeast')
...
%     saveas(gcf,strcat(output,'Hbpeak.png'))
    ...
    if ~isempty(jrgeco1a)
        ...
        traceImage(jrgeco1a_downsampled_blocks,jrgeco1aCorr_downsampled_blocks,red_downsampled_blocks,...
            'FAD_cam2','FADCorr_cam2','625nm Reflectance',jrgeco1a_active,jrgeco1aCorr_active,red_active,'m','k','r',ROI,'\DeltaF/F%',...
