function [spectraAllMice,runFreq,speciesList] = examplePowerAnalysis_Delta(excelFile, rows)
...
for runInfo = runsInfo
    ...
    if exist(saveFilePowerData, 'file')
        ...
        [spectraFigure, mapFigure] = plotPower_Delta(fRange,runFreq,runSpectra,runMeanSpectra,...
...
%% Average all of the runs for each mouse
    ...
    if lastRunInMouse
        ...
        [numRuns, numSpecies] = size(freqMouse);
            ...
            [mouseAvgSpectraFig, mouseAvgMapFigure] = plotPower_Delta(fRange,freqMouseAvg,...
...
if numMice > 1
    ...
    [mouseAvgSpectraFig, mouseAvgMapFigure] = plotPower_Delta(fRange,freqAllMouseAvg,...
