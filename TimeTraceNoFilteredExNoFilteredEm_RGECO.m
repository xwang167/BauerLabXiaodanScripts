...
for excelRow = excelRows
    ...
    saveDir_corrected = fullfile('X:\XW\NoFilteredEmEX\RGECO_Timetrace',recDate);
...
save(fullfile('X:\XW\NoFilteredEmEX\RGECO_Timetrace\cat',strcat(recDate,'-',miceName,'-stim')),...
...
xlabel('Time(s)')
ylim([-0.035 0.0525])
title(strcat(recDate,'-RGECO-stim-Ex w/o filter, Em w/o filter'))
legend('HBO','HbR','jRGECO1a','FAD','location','northeast')
output = fullfile('X:\XW\NoFilteredEmEX\RGECO_Timetrace\cat',strcat(recDate,'-',miceName,'-stim'));
saveas(gcf,strcat(output,'timetrace.fig'))
saveas(gcf,strcat(output,'timetrace.png'))
close all