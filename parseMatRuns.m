function runsInfo = parseMatRuns(excelFile,excelRows)
%parseDatRuns Parses information from excel sheet about runs saved as .dat files and
...
%   files.
%
%   System (required) = which system is used? This is the input to
...
for row = tableRows % for each row of excel file
    ...
    for ind = 1:numel(validFiles)
        runNumStart = strfind(validFiles(ind).name,['-' sessionType]);
        runNumStart = runNumStart + numel(sessionType) + 1;
        
        runNumEnd = find(isstrprop(validFiles(ind).name,'digit'));
        runNumEnd(runNumEnd < runNumStart) = [];
        runList(ind) = str2double(validFiles(ind).name(runNumStart:runNumEnd));
    end
    ...
    for run = uniqueRuns
        ...
        runFileList = cell(1,numel(runFiles));
        for file = 1:numel(runFiles)
            runFileList{file} = fullfile(runFiles(file).folder,runFiles(file).name);
        end
        ...
        runNumStart = strfind(runFiles(1).name,'-');
        saveFileName = runFiles(1).name; saveFileName = saveFileName(1:runNumStart(end-1)-1);
        ...
        runsInfo(totalRunInd).rawFile = runFileList;
