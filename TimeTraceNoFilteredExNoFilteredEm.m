...
FAD2Corr_timetrace_mice = NaN(750,5);
...
for excelRow = excelRows
    ...
    saveDir_corrected = fullfile('X:\XW\NoFilteredEmEX\WT_Timetrace',recDate);
    ...
    FAD2Corr_timetrace_mouse = NaN(750,3);
    ...
    for n = runs
        ...
        clear xform_datahb xform_FAD2 xform_FAD
        ...
        FAD2Corr_timetrace = mouse.physics.correctHb(FAD2_timetrace,datahb_timetrace,[E_in(1) E_out(1)],[E_in(2) E_out(2)],dpIn,dpOut);
        ...
        baseline_FAD2Corr = mean(FAD2Corr_timetrace(1:sessionInfo.stimbaseline,:),1);
        ...
        save(fullfile(saveDir_corrected,timetraceName),...
            'datahb_timetrace','FAD2Corr_timetrace','FADCorr_timetrace')
        ...
        FAD2Corr_timetrace_mouse(:,n) = FAD2Corr_timetrace_mean;
        ...
        plot((1:length(FADCorr_timetrace_mean))/25,FAD2Corr_timetrace_mean,'m-')
        hold on
        plot((1:length(FADCorr_timetrace_mean))/25,FADCorr_timetrace_mean,'g-')
        xlabel('Time(s)')
        ylim([-0.035 0.07])
        title(strcat(recDate,'-', mouseName,'-',sessionType,num2str(n),'-Ex w/o filter, Em w/o filter'))
        legend('HBO','HbR','FAD cam2','FAD cam1','location','northeast')
    ...
    FAD2Corr_timetrace_mouse = mean(FAD2Corr_timetrace_mouse,2);
    FADCorr_timetrace_mouse = mean(FADCorr_timetrace_mouse,2);
    save(fullfile(saveDir_corrected,timetraceName_mouse),...
        'datahb_timetrace_mouse','FAD2Corr_timetrace_mouse','FADCorr_timetrace_mouse')
    figure
    yyaxis left
    ylabel('Hb(M)')
    ...
    yyaxis right
    ylabel('Fluorescence(\DeltaF/F)')
    plot((1:length(FADCorr_timetrace_mouse))/25,FAD2Corr_timetrace_mouse,'m-')
    hold on
    plot((1:length(FADCorr_timetrace_mouse))/25,FADCorr_timetrace_mouse,'g-')
    xlabel('Time(s)')
    ylim([-0.035 0.07])
    title(strcat(recDate,'-', mouseName,'-',sessionType,'-Ex w/o filter, Em w/o filter'))
    legend('HBO','HbR','FAD cam2','FAD cam1','location','northeast')
    ...
    FAD2Corr_timetrace_mice(:,ll) = FAD2Corr_timetrace_mouse;
    FADCorr_timetrace_mice(:,ll)= FADCorr_timetrace_mouse;
    ll = ll + 1;
    
    clearvars -except excelFile excelRows runs  miceName ll datahb_timetrace_mice FAD2Corr_timetrace_mice FADCorr_timetrace_mice recDate
end

datahb_timetrace_mice = mean(datahb_timetrace_mice,3);
FAD2Corr_timetrace_mice = mean(FAD2Corr_timetrace_mice,2);
FADCorr_timetrace_mice = mean(FADCorr_timetrace_mice,2);
save(fullfile('X:\XW\NoFilteredEmEX\WT_Timetrace\cat',strcat(recDate,'-',miceName,'-stim')),...
    'datahb_timetrace_mice','FAD2Corr_timetrace_mice','FADCorr_timetrace_mice')
figure
yyaxis left
...
ylabel('Fluorescence(\DeltaF/F)')
hold on
plot((1:length(FADCorr_timetrace_mice))/25,FAD2Corr_timetrace_mice,'m-')
hold on
plot((1:length(FADCorr_timetrace_mice))/25,FADCorr_timetrace_mice,'g-')
xlabel('Time(s)')
ylim([-0.035 0.0525])
title(strcat(recDate,'-WT-stim-Ex w/o filter, Em w/o filter'))
legend('HBO','HbR','FAD cam2','FAD cam1','location','northeast')
output = fullfile('X:\XW\NoFilteredEmEX\WT_Timetrace\cat',strcat(recDate,'-',miceName,'-stim'));
saveas(gcf,strcat(output,'timetrace.fig'))
saveas(gcf,strcat(output,'timetrace.png'))
close all