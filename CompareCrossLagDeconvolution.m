...
subplot(121)
for ii = 1:44
scatter(mean(T_HRF(ii)),lagTime_NVC(ii),"MarkerEdgeColor",raw(colorOrder(ii)*scale,:),"MarkerFaceColor",raw(colorOrder(ii)*scale,:))
hold on
end
title('NVC')
xlabel('Deconvolution')
ylabel('Cross Correlation')
xlim([0.5 1.2])
ylim([0.5 1.2])
subplot(122)
for ii = 1:44
scatter(mean(T_MRF(ii)),lagTime_NMC(ii),"MarkerEdgeColor",raw(colorOrder(ii)*scale,:),"MarkerFaceColor",raw(colorOrder(ii)*scale,:))
hold on
end
title('NMC')
xlabel('Deconvolution')
ylabel('Cross Correlation')
sgtitle('Regional Comparison of Time to Peak b/w Deconvolution and Cross Correlaiton')
xlim([-0.018 0.15])
ylim([-0.018 0.15])