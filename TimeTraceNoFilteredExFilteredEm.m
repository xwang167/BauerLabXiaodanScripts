...
for excelRow = excelRows
    ...
    saveDir_corrected = fullfile('X:\XW\NoFilteredEx-FilteredEm\WT_Timetrace',recDate);
    ...
    for n = runs
        ...
        FADCorr_timetrace = reshape(FADCorr_timetrace,[],10);
              ...
              baseline_datahb = mean(datahb_timetrace(:,1:125,:),2);
...
save(fullfile('X:\XW\NoFilteredEx-FilteredEm\WT_Timetrace\cat',strcat(recDate,'-',miceName,'-stim')),...
...
xlabel('Time(s)')
ylim([-0.035 0.0525])
title(strcat(recDate,'-WT-stim-Ex w/o filter, Em w/ filter'))
legend('HBO','HbR','FAD cam2','FAD cam1','location','northeast')
output = fullfile('X:\XW\NoFilteredEx-FilteredEm\WT_Timetrace\cat',strcat(recDate,'-',miceName,'-stim'));
saveas(gcf,strcat(output,'timetrace.fig'))
saveas(gcf,strcat(output,'timetrace.png'))
close all