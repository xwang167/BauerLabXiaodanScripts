...
for excelRow = excelRows
    ...
    for ii = 1:3
        ...
        subplot(1,3,ii)
        plot((1:750)/25,retrosplenial(1:750))
        xlabel('Time(sec)')
        ylabel('Fluorescence(\DeltaF/F)')
        ylim([-0.1 0.5])
        title(['fc' num2str(ii)])
    ...
    savefig(gcf,fullfile(saveDir,strcat(recDate,'-',mouseName,'-',sessionType,'-ClusterTimeTrace_visual','.fig')))
    saveas(gcf,fullfile(saveDir,strcat(recDate,'-',mouseName,'-',sessionType,'-ClusterTimeTrace_visual','.png')))
    close all
end
...
for excelRow = excelRows
    ...
    savefig(gcf,fullfile(saveDir,strcat(recDate,'-',mouseName,'-',sessionType,'-ClusterTimeTrace_retrosplenial','.fig')))
    saveas(gcf,fullfile(saveDir,strcat(recDate,'-',mouseName,'-',sessionType,'-ClusterTimeTrace_retrosplenial','.png')))
    close all
end