...
for row = tableRows % for each row of excel file
    ...
    if contains(system,'EastOIS2') %xw 220602 EastOIS2_FAD_RGECO to EastOIS2
        for file = 1:numel(D)
    ...
    for run = uniqueRuns
        ...
        if isempty(darkFramesInd)
            ...
            runsInfo(totalRunInd).totNum_frame=runTime*samplingRate*numCh+(numCh*darkFramesInd(end)); %jpc added %xw 220604 not work for darkFrames = [];
        end
