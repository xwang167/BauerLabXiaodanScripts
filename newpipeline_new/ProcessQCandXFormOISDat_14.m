function ProcessQCandXFormOISDat_14(excelFile,excelRows)
...
for runInd=mouse_ind %for each mouse
    ...
    % xw 220605 for optogentics project, sometime the mask and landmarks will be madeduring acquisition, not processing
    ...
    if  ~contains(runInfo.system,'EastOIS2')
        ...
        if ~exist(runInfo.saveMaskFile,'file') || (~ismember('xform_StimROIMask', variableInfo) && runInfo.session =="stim") %%xw 220603 for laser case, we have a landmarksandMask without stimMask beofre the processing script
            raw = readtiff(runInfo.rawFile{1},300*runInfo.numCh);% xw 220624 EastOIS1_FAD has 400 dark frames in total
            ...
            if runInfo.session =="stim"
                ...
                for jj=1:runInfo.numCh
                    if isempty(runInfo.darkFramesInd)% XW 220605 where there is no darkframe
                        tmp(:,:,jj)=(mean(raw(:,:,jj,runInfo.stimStartTime*runInfo.samplingRate+1:end),4)-...
                        mean(raw(:,:,jj,1:runInfo.stimStartTime*runInfo.samplingRate),4)).^2; % xw220605 1+runInfo.stimStartTime*runInfo.samplingRate to runInfo.stimStartTime*runInfo.samplingRate
                    else
                    tmp(:,:,jj)=(mean(raw(:,:,jj,runInfo.stimStartTime*runInfo.samplingRate+runInfo.darkFramesInd(end)+1:end),4)-...
                        mean(raw(:,:,jj,runInfo.darkFramesInd(end)+1:runInfo.darkFramesInd(end)+runInfo.stimStartTime*runInfo.samplingRate),4)).^2; %mean from dark frames to end % xw220605 1+runInfo.stimStartTime*runInfo.samplingRate to runInfo.stimStartTime*runInfo.samplingRate
                    end
            ...
            if ~(runInfo.session=="stim")
                ...
                else  %xw 220605 for optogentics project, sometime the mask and landmarks will be madeduring acquisition, not processing
        ...
        if ~exist(runInfo.saveMaskFile,'file') || (~ismember('xform_StimROIMask', variableInfo) && runInfo.session =="stim")%%xw 220603 for laser case, we have a landmarksandMask without stimMask beofre the processing script
            ...
            if length(size(tmp.raw_unregistered))==4 %xw 220603raw_unregistered use to be 128*128*channel*frame
                raw_unregistered=tmp.raw_unregistered(:,:,:,1:200);
            else
...
%                 if (StimCh~= 2) && (runInfo.session=="stim") %if there is a dropped frame
                    WL(:,:,2) = fixed;
                    WL(:,:,3) = fixed;
%                     WL(:,:,2) = 2.8*fixed; %add some conrast!%xw 220610 registered to fix
%                     WL(:,:,3) = 4*fixed;   %add some conrast!%xw 220610 registered to fix
%                 else
%                     WL(:,:,2) = fixed;
%                     WL(:,:,3) = fixed;
%                 end
                ...
                if ~exist('I','var') %% for some period of time I was changed to affineMarkers
                    I = affineMarkers;
                end
                xform_StimROIMask=affineTransform(StimROIMask,I);
                xform_StimROIMask=single(uint8(xform_StimROIMask));
...
for runInd = 1:runNum
    ...
    elseif ((exist(stimExist) && exist(stimExist2) && strcmp(runInfo.session,'stim') ) ||  (exist(fcExist) && strcmp(runInfo.session,'fc') ))
        ...
        if ~contains(runInfo.system,'EastOIS2')
            ...
            raw_unregistered = reshape(raw_unregistered,128,128,[]); %% xw 220602 reshape because in the past we save raw_unregistered as 128*128*channels*frames
    ...
    if runInfo.session =="fc"
        ...
        %Initialize
        %Building Data Full
        data_full = nan(size(xform_datahb,1),size(xform_datahb,2),size(xform_datahb,4),length(runInfo.Contrasts));%if statements to length(runInfo.Contrasts)
        ...
        if strcmp(runInfo.system,'EastOIS1+laser')
            data_full(:,:,:,4) = xform_datalaser;
        elseif strcmp(runInfo.system,'EastOIS1_FAD')
            data_full(:,:,:,4) = xform_dataFADCorr;
        elseif strcmp(runInfo.system,'EastOIS1_GCaMP')
            data_full(:,:,:,4) = xform_datafluorCorr;
        elseif strcmp(runInfo.system,'EastOIS2_FAD_RGECO')
            data_full(:,:,:,4) = xform_datafluorCorr;
            data_full(:,:,:,5) = xform_dataFADCorr;
        elseif strcmp(runInfo.system,'EastOIS2+laser')
            data_full(:,:,:,4) = xform_datafluorCorr;
        ...
        for ii=1:size(data_full,4)
            ...
            data_isa(:,:,:,ii)=resample(squeeze(data_isa_tmp(:,:,:,ii)), (2*fRange_ISA(2)*1.25)*5 ,runInfo.samplingRate*5,'Dimension',3);
        ...
        if isempty(runInfo.FADChInd) && isempty(runInfo.fluorChInd)
            ContFig2Save= {'HbO','HbT'};
        end
        
        if isempty(runInfo.FADChInd) && ~isempty(runInfo.fluorChInd)
            ContFig2Save= {'HbO','HbT','Calcium'};
        end
        
        if ~isempty(runInfo.FADChInd) && isempty(runInfo.fluorChInd)
            ContFig2Save= {'HbO','HbT','FAD'};
        end
        
        if ~isempty(runInfo.FADChInd) && ~isempty(runInfo.fluorChInd)
            ContFig2Save= {'HbO','HbT','Calcium','FAD'};
        end
    ...
    elseif runInfo.session =="stim"
        ...
        [fh,peakMaps] = generateBlockMap_Dynamics(data_full_BaselineShift,data_full_gsr_BaselineShift,runInfo,numBlocks,peakTimeRange,xform_isbrain,xform_StimROIMask); % xw 220611 need peakMaps to find M1b
        %save
        ...
        export_fig(strcat(saveName,'.png'), '-transparent') %xw 220601 from saveas to export_fig because the plots range is bigger than xlim
        close all
