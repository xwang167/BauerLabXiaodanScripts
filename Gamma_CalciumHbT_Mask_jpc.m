...
for excelRow = excelRows
    ...
    for n = runs
        ...
        load(fullfile(saveDir,processedName),'xform_datahb','xform_jrgeco1aCorr','lagTimeTrial_HbTCalcium')
...
%         
        ...
        [T,W,A,r,r2,hemoPred] = interSpeciesGammaFit_CalciumHbT_Mask_jpc(Calcium_filter*100,HbT_filter*10^6,t,mask,lagTimeTrial_HbTCalcium);
        toc
        if exist(fullfile(saveDir,saveName))
save(fullfile(saveDir,saveName),'T','W','A','r','r2','hemoPred','-append')
        else
            save(fullfile(saveDir,saveName),'T','W','A','r','r2','hemoPred','-v7.3')
        end
        figure
        subplot(2,3,1)
        imagesc(T,[0,2])
        ...
        imagesc(xform_WL,'AlphaData',1-mask);
        set(gca,'FontSize',14,'FontWeight','Bold')
        sgtitle('No Normalization')

        saveas(gcf,fullfile(saveDir,strcat(recDate,'-',mouseName,'-',sessionType,num2str(n),'_CalciumHbT_GammaFit_nan.png')));
        saveas(gcf,fullfile(saveDir,strcat(recDate,'-',mouseName,'-',sessionType,num2str(n),'_CalciumHbT_GammaFit_nan.fig')));
...
for excelRow = excelRows
    ...
    for n = runs
        visName = strcat(recDate,'-',mouseName,'-',sessionType,num2str(n));
        saveName =  strcat(recDate,'-',mouseName,'-',sessionType,num2str(n),'_nan.mat');
        load(fullfile(saveDir,saveName),'T','W','A','r','r2','hemoPred')
    ...
    processedName_mouse = strcat(recDate,'-',mouseName,'-',sessionType,'GammaHbT_nan','.mat');
    save(fullfile(saveDir,processedName_mouse),'T_mouse','W_mouse','A_mouse','r_mouse','r2_mouse','hemoPred_mouse','-v7.3')
    ...
    saveas(gcf,fullfile(saveDir,strcat(recDate,'-',mouseName,'-',sessionType,'_CalciumHbT_GammaFit_nan.png')));
    saveas(gcf,fullfile(saveDir,strcat(recDate,'-',mouseName,'-',sessionType,'_CalciumHbT_GammaFit_nan.fig')));
...
T_mice = nanmean(T_mice,3);
W_mice = nanmean(W_mice,3);
A_mice = nanmean(A_mice,3);
r_mice = nanmean(r_mice,3);
r2_mice = nanmean(r2_mice,3);
hemoPred_mice = nanmean(hemoPred_mice,4);

processedName_mice = '191030--R5M2285-R5M2286-R5M2288-R6M2460-awake-R6M1-awake-R6M2497-awake-fc_nan.mat';
save(fullfile('L:\RGECO\cat',processedName_mice),'T_mice','W_mice','A_mice','r_mice','r2_mice','hemoPred_mice','-v7.3')
mask = logical(mask_new.*xform_isbrain_mice);
figure
subplot(2,3,1)
imagesc(T_mice,[0,2])
colorbar
...
hemoPred_mice = nanmean(hemoPred_mice,4);
mask = logical(mask_new.*xform_isbrain_mice);
processedName_mice = '191030--R5M2286-anes-R5M2285-anes-R5M2288-anes-R6M2460-anes-R6M1-anes-R6M2497-anes-fc_nan.mat';
save(fullfile('L:\RGECO\cat',processedName_mice),'T_mice','W_mice','A_mice','r_mice','r2_mice','hemoPred_mice','-v7.3')
...
imagesc(xform_WL,'AlphaData',1-mask);
set(gca,'FontSize',14,'FontWeight','Bold')
saveas(gcf,fullfile('L:\RGECO\cat',strcat('191030--R5M2286-anes-R5M2285-anes-R5M2288-anes-R6M2460-anes-R6M1-anes-R6M2497-anes-fc','_CalciumHbT_GammaFit_nan.png')));
saveas(gcf,fullfile('L:\RGECO\cat',strcat('191030--R5M2286-anes-R5M2285-anes-R5M2288-anes-R6M2460-anes-R6M1-anes-R6M2497-anes-fc','_CalciumHbT_GammaFit_nan.fig')));
% 
% for excelRow = excelRows
%     [~, ~, excelRaw]=xlsread(excelFile,1, ['A',num2str(excelRow),':V',num2str(excelRow)]);
%     recDate = excelRaw{1}; recDate = string(recDate);
