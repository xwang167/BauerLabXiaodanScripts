...
for excelRow = excelRows
    ...
    for n = runs
        ...
        for xInd = 1:size(HbT_filter,2)
            for yInd = 1:size(HbT_filter,1)
                if mask(yInd,xInd)
                    ...
                    fcn = @(param)he.fcn(param);
                    [~,pixHrfParam,obj_val,~,~] = evalc('fminsearchbnd(fcn,[1,1.5,0.25],[0.01,0.3,0],[2,3,1],options)');
        ...
        if exist(fullfile(saveDir,strcat(recDate,'-',mouseName,'-',sessionType,num2str(n),'_jonahA0','.mat')),'file')
            save(fullfile(saveDir,strcat(recDate,'-',mouseName,'-',sessionType,num2str(n),'_jonahA0','.mat')),...
        ...
        saveas(gcf,fullfile(saveDir,strcat(recDate,'-',mouseName,'-',sessionType,num2str(n),'_CalciumHbT_GammaFit_jonahA0.png')));
        saveas(gcf,fullfile(saveDir,strcat(recDate,'-',mouseName,'-',sessionType,num2str(n),'_CalciumHbT_GammaFit_jonahA0.fig')));  
