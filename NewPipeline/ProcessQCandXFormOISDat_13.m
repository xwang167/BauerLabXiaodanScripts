...
excelFile="K:\hypoglycemia\Hypoglycemia.xlsx";
excelRows=[2];
...
for runInd=mouse_ind %for each mouse
    ...
    if  ~contains(runInfo.system,'EastOIS2')
        ...
        if ~exist(runInfo.saveMaskFile,'file')
            raw = readtiff(runInfo.rawFile{1},200*runInfo.numCh);
            ...
            if runInfo.session =="stim"
                ...
                for jj=1:runInfo.numCh
                    tmp(:,:,jj)=(mean(raw(:,:,jj,runInfo.stimStartTime*runInfo.samplingRate+runInfo.darkFramesInd(end)+1:end),4)-...
                        mean(raw(:,:,jj,runInfo.darkFramesInd(end)+1:1+runInfo.darkFramesInd(end)+runInfo.stimStartTime*runInfo.samplingRate),4)).^2; %mean from dark frames to end
        ...
        if ~exist(runInfo.saveMaskFile,'file')
            ...
            WL = zeros(128,128,3);
            WL(:,:,1) = registered;
            if runInfo.session=="stim" %if there is a dropped frame%%xw
                if StimCh~= 2
...
for runInd = 1:runNum
    ...
    if exist( runInfo.saveHbFile,'file')  && ~(exist(stimExist) ||  exist(fcExist) )
        disp([runInfo.saveHbFile,' Already pre-processed, Processing Stim/FC'])
    ...
    elseif (exist(stimExist) ||  exist(fcExist) )
        ...
        if ~isempty(runInfo.fluorChInd)
            ...
            elseif contains(runInfo.system,'EastOIS2_FAD_RGECO') %jRGECGO1a correction
