...
%     new_order(new_order==0)=runInfo.numCh;
    ...
    DarkFrame_data=DarkFrame_data; %reorder the channels. If dropped frame in channel 4 then channel 4 becomes channel 1.%%xw???
    ...
    fixed_by_channel(:,:,runInfo.darkFramesInd(end)+1:end)=reshape(NonDarkFrame_data,OGsize(1)*OGsize(2),runInfo.numCh,[]);%xw????
    fixed_by_channel=reshape(fixed_by_channel,OGsize(1),OGsize(2),runInfo.numCh,[]);
end
end