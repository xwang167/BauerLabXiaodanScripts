function [rawFig, displacement] = qcRaw_xw(runInfo,time,raw,representativeCh,isbrain,droppedFrames,system,subfile)
...
if isempty(droppedFrames)
    ...
    runInfo.session '' num2str(runInfo.run) subfile '-rawQCData.mat'];
else
    saveLoc = [runInfo.saveFolder filesep runInfo.recDate '-' runInfo.mouseName '-'...
    runInfo.session '' num2str(runInfo.run) subfile '-rawQCData_dropCorr.mat'];
end
...
mouseInfo = [runInfo.recDate, '-', runInfo.mouseName, '-', runInfo.session, char(string(runInfo.run)) '_' subfile];
...
elseif strcmp(system,'EastOIS2+laser')
    ...
    elseif strcmp(system,'EastOIS2+laser+grid')
    Colors=[0 1 0; 1 0 0; 1 0 1];
    TickLabels={'G', 'R', 'C'};
    legendName = {'green LED', 'red LED', 'RGECO'};
end
for ch = 1:numCh
