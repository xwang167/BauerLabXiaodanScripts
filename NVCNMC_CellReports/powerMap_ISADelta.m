...
%   figure('units','normalized','outerposition',[0 0 0.5 0.7]);
%     subplot(2,3,1)
    powerMapVis_absolute(jrgeco1aCorr_ISA_powerMap_mice,'(\DeltaF/F%)',-2,-1)
    title('Corr jRGECO1a ISA')
    %subplot(2,3,2)
    powerMapVis_absolute(FADCorr_ISA_powerMap_mice,'(\DeltaF/F%)',-3,-2)
     title('Corr FAD ISA')    
   % subplot(2,3,3)   
    powerMapVis_absolute(total_ISA_powerMap_mice,'(\muM)',-2.5,-0.5)    
    title('Total ISA')    
   % subplot(2,3,4)
     powerMapVis_absolute(jrgeco1aCorr_Delta_powerMap_mice,'(\DeltaF/F%)',-3.4,-1.2)
    title('Corr jRGECO1a Delta')
   % subplot(2,3,5)    
    powerMapVis_absolute(FADCorr_Delta_powerMap_mice,'(\DeltaF/F%)',-4.4,-3.4)
     title('Corr FAD Delta')   
    %subplot(2,3,6)
     powerMapVis_absolute(total_Delta_powerMap_mice,'(\muM)',-3.8,-3)
     suptitle('Awake Power Map')
     title('Total  Delta')
...
function powerMapVis_absolute(powerMap,unit,minVal,maxVal)
