function QCcheck_fftVis_substract(hz, leftData,rightData,leftLabel,rightLabel,leftLineStyle,rightLineStyle,legendName,saveDir,Name)
...
for ii = 1 : size(leftData,1)
    yyaxis left
    semilogx(hz,10*log10(leftData{ii}-interp1(hz,leftData{ii},0.01)+10),leftLineStyle{ii});
...
%     else
    semilogx(hz,10*log10(rightData{ii}-interp1(hz,rightData{ii},0.01)+10),rightLineStyle{ii})
%     end
    hold on
end
%uistack(a,'top')
