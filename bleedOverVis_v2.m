function bleedOverVis_v2(excelFile,excelRows,runs,miceName,saveDir_cat,nVx,nVy,camInd,ROI)
import mouse.*
...
for excelRow = excelRows
    ...
    for n = runs
        ...
        if strcmp(systemType,'EastOIS2')
            if strcmp(sessionInfo.mouseType,'jrgeco1a-opto3')
                rawName_OneCam = strcat(recDate,'-',mouseName,'-',sessionType,num2str(n),'-cam', num2str(camInd), '.mat');
                load(fullfile(rawdataloc,recDate,rawName_OneCam))             
            ...
            elseif strcmp(sessionInfo.mouseType,'Gopto3')||strcmp(sessionInfo.mouseType,'Wopto3')||strcmp(sessionInfo.mouseType,'PV')
                ...
                rawName_OneCam = strcat(recDate,'-',mouseName,'-',sessionType,num2str(n),'-cam1.mat');
                load(fullfile(rawdataloc,recDate,rawName_OneCam))
    ...
    save(fullfile(saveDir,strcat(recDate,mouseName,'-cam',num2str(camInd))),'raw_mouse');
    raw_mice(:,:,:,:,mouseInd)= raw_mouse;
    mouseInd = mouseInd+1;
end
raw_mice = squeeze(mean(raw_mice,5));
save(fullfile(saveDir_cat,strcat(recDate,'-',miceName,'cam1-MiceRawData.mat')),'raw_mice')
if ~exist('ROI', 'var')
...
texttitle = strcat(miceName,'Averaged counts for Green LED from Cam',num2str(camInd), 'under NonOverlap Mode');
if camInd == 1
...
texttitle = strcat(miceName,'Averaged counts for Red LED from Cam',num2str(camInd), 'under NonOverlap Mode');
if camInd == 1
...
texttitle = strcat(miceName,'Averaged counts for Bright Green LED from Cam',num2str(camInd), 'under NonOverlap Mode');
if camInd == 1
    ...
    quanti_bleedover(raw_mice(:,:,3,:),stimBaselineFrames,stimDurationFrames,1,sessionInfo.framerate,ROI,1*10^4,3*10^5,500,texttitle,output)
end

texttitle = strcat(miceName,'Averaged counts for Laser from Cam',num2str(camInd),' under NonOverlap Mode');
if camInd == 1
    output = fullfile(saveDir_cat,strcat(recDate,'-',miceName,'Laser-Cam1'));
    quanti_bleedover(raw_mice(:,:,4,:),stimBaselineFrames,stimDurationFrames,1,sessionInfo.framerate,ROI,2500,4500,10000,texttitle,output)        
else
