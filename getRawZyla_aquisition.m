function raw_unregistered = getRawZyla_aquisition(folderDir,folderName,numTotalFrames,numLED,nVx,nVy,mouseType)                 
...
a=0;
for i=1:size(tempdir2, 1);
    fid=fopen(fullfile(folderName, tempdir2(i).name));
    fseek(fid,0,'bof');%%????
    for n=1:imperfile;
        a=a+1;
        if strcmp(pixelencoding,'Mono16')
...
delete(fullfile(folderDir,folderName))
save(fullfile(folderDir,strcat(folderName,'.mat')),'raw_unregistered')
