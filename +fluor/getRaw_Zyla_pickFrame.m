function rawdata = getRaw_Zyla_pickFrame(rawdatafolder,nVx,nVy,numTotalFrames,numLED,mouseType)
...
for i=1:size(tempdir2, 1);
    ...
    for n=1:imperfile;
        ...
        if camIndex == 1
            strcmp(mouseType,'gcamp6f')
            tempdata1=fread(fid,xPixels*yPixels,pixelencode);
            
            tempdata1 =reshape(tempdata1, xPixels, yPixels);
            tempdata1=rot90(reshape(tempdata1, xPixels, yPixels), 1); % assumes data is square, equal pixels in x and y
            tempdata1 = flip(tempdata1,2);
