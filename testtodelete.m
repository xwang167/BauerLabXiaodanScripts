...
BilatInhibition = nan(160,1);
for ii = 1:160
    if seedmapLocsOG(1,ii) ~= 0 && seedmapLocsOG(2,ii) ~= 0
        x1 = 129-seedmapLocsOG(2,ii);
        ...
        BilatInhibition(ii) = nanmedian(inhibitionMap_Calcium_NoGSR_mice(ROI,ii));
    end
end
...
for i=1:length(seedmapLocs)
    if seedmapLocs(1,i) ~= 0 && seedmapLocs(2,i) ~= 0
        brainMap(seedmapLocs(1,i)-pitch:seedmapLocs(1,i)+pitch,seedmapLocs(2,i)-pitch:seedmapLocs(2,i)+pitch) = BilatInhibition(i);
    end
end


subplot(121)
imagesc(brainMap,[-0.7 0.7]);
...
for i=1:length(seedmapLocs)
    if seedmapLocs(1,i) ~= 0 && seedmapLocs(2,i) ~= 0
        brainMapOG(seedmapLocs(1,i)-pitch:seedmapLocs(1,i)+pitch,seedmapLocs(2,i)-pitch:seedmapLocs(2,i)+pitch) = BilatInhibition(i);
    end
end
imagesc(brainMapOG)
axis square image off
...
imagesc(brainMap, "AlphaData", brainMapAD,[-0.7 0.7]);
axis square image off
h = colorbar;
ylabel(h, 'Calcium(\DeltaF/F%)','fontsize',10,'FontWeight','bold')
a = get(gca,'XTickLabel');
set(gca,'XTickLabel',a,'fontsize',10,'FontWeight','bold')
title('Calcium Bilateral Inhibition Magnitude')
...
imagesc(brainMapOG, "AlphaData", brainMapADOG,[-0.7 0.7]);
axis square image off
h = colorbar;
ylabel(h, 'Calcium(\DeltaF/F%)','fontsize',10,'FontWeight','bold')
a = get(gca,'XTickLabel');
set(gca,'XTickLabel',a,'fontsize',10,'FontWeight','bold')
title('Calcium Bilateral Inhibition Magnitude, Actual Location')
sgtitle(miceName(2:end))

save('X:\PVChR2-Thy1RGECO\cat\N13M309-N13M549-N13M548-N27M753-N27M755-N26M761-N26M764-N24M783-N24M778-N24M322-BilateralInhibitionMagnitude.mat',...
    'BilatInhibition')