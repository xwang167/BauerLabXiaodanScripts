...
excelRows =  304:311;%[462 464 466];%[367,371,375,397,401,409];%,229,233,237%,
excelFile = "C:\Users\xiaodanwang\Documents\GitHub\BauerLabXiaodanScripts\DataBase_Xiaodan.xlsx";
numMice = length(excelRows);

miceName = [];
catDir = 'K:\OptoRGECO\cat' ;
...
%         xform_Laser_mice = cat(4,xform_Laser_mice,xform_Laser);
        ...
        sessionInfo.stimblocksize = excelRaw{11};
        load(fullfile(saveDir,processedName_mouse),...
            'xform_datahb_mouse_GSR_filtered','xform_jrgeco1a_mouse_GSR','xform_jrgeco1aCorr_mouse_GSR','xform_red_mouse_GSR','ROI_NoGSR')
        if size(xform_datahb_mouse_GSR_filtered,1)>1
            xform_datahb_mice_GSR = cat(5,xform_datahb_mice_GSR,xform_datahb_mouse_GSR_filtered);
        end
        clear xform_datahb_mouse_GSR
        xform_jrgeco1a_mice_GSR = cat(4,xform_jrgeco1a_mice_GSR,xform_jrgeco1a_mouse_GSR);
        clear xform_jrgeco1a_mouse_GSR
        ...
        load(fullfile(saveDir,processedName_mouse),...
            'xform_datahb_mouse','xform_jrgeco1a_mouse','xform_jrgeco1aCorr_mouse','xform_red_mouse')
        xform_datahb_mice_NoGSR = cat(5,xform_datahb_mice_NoGSR,xform_datahb_mouse);
        clear xform_datahb_mouse_NoGSR
        xform_jrgeco1a_mice_NoGSR = cat(4,xform_jrgeco1a_mice_NoGSR,xform_jrgeco1a_mouse);
        clear xform_jrgeco1a_mouse_NoGSR
        xform_jrgeco1aCorr_mice_NoGSR = cat(4,xform_jrgeco1aCorr_mice_NoGSR,xform_jrgeco1aCorr_mouse);
        clear xform_jrgeco1aCorr_mouse_NoGSR
        xform_red_mice_NoGSR = cat(4,xform_red_mice_NoGSR,xform_red_mouse);
        clear xform_red_mouse_NoGSR
...
processedName_mice = strcat(recDate,'-',miceName,'-',sessionType,'-allBlocks','.mat');
...
elseif strcmp(char(sessionInfo.miceType),'jrgeco1a-opto3')
    ...
    load('K:\OptoRGECO\cat\200217-ChR2-RGECO-Awake-stim-allBlocks.mat','ROI');
    texttitle_NoGSR = strcat(miceName,'-stim'," ",'without GSR nor filtering');
    output_NoGSR= fullfile(catDir,strcat(recDate,'-',miceName,'-stim','_NoGSR'));
...
%     clear xform_datahb_mice_NoGSR xform_jrgeco1a_mice_NoGSR xform_jrgeco1aCorr_mice_NoGSR xform_red_mice_NoGSR  xform_FAD_mice_NoGSR xform_FADCorr_mice_NoGSR xform_green_mice_NoGSR
    ...
    save(fullfile(catDir,processedName_mice),'xform_jrgeco1a_mice_GSR','xform_jrgeco1aCorr_mice_GSR','xform_red_mice_GSR','ROI','-append')
    ...
    [goodBlocks_GSR] = QC_stim_range(squeeze(xform_datahb_mice_GSR(:,:,1,:))*10^6,squeeze(xform_datahb_mice_GSR(:,:,2,:))*10^6,...
    ...
    texttitle_NoGSR = strcat(miceName,'-stim'," ",'without GSR nor filtering');
    output_NoGSR= fullfile(catDir,strcat(recDate,'-',miceName,'-stim','-NoGSR'));
    
    disp('QC on non GSR stim')
    
    QC_stim_range(squeeze(xform_datahb_mice_NoGSR(:,:,1,:))*10^6,squeeze(xform_datahb_mice_NoGSR(:,:,2,:))*10^6,...
