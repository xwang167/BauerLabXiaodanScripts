...
for excelRow = [35 36]%7:15
    ...
    for run = 1:3
        rawFileName = strcat(recDate,'-',mouseName,'-',sessionType,num2str(run),'.mat');
        rawFileName = fullfile(saveDir,'Combined',rawFileName);
        

        
        load(rawFileName,'raw')
        rawdata = double(raw);
        
        
        [info.nVy, info.nVx,systemInfo.LEDFiles, frames]=size(rawdata);
%         systemInfo.LEDFiles = 3;
...
%     
        ...
        info.freqout = 1;
        load(fullfile(saveDir,'Combined',strcat(recDate,'-',mouseName,'-LandmarksandMarks','.mat')),'isbrain')
        ...
        if strcmp(system, 'fcOIS1')
            BlueChan=4;
        elseif strcmp(system, 'fcOIS2')||strcmp(system, 'EastOIS1')||strcmp(system, 'EastOIS1_Fluor')||strcmp(system, 'EastOIS2')
            BlueChan=1;
        end
        ...
        annotation('textbox',[0.125 0.95 0.75 0.05],'HorizontalAlignment','center','LineStyle','none','String',strcat(recDate,'-',mouseName,'-',sessionType,num2str(run),' Raw Data Visualization'),'FontWeight','bold','Color',[1 0 0]);
        
        output=strcat(fullfile(saveDir,strcat(recDate,'-',mouseName,'-',sessionType,num2str(run))),'_RawDataVis.jpg');
        orient portrait
        print ('-djpeg', '-r300', output);
        
        figure('visible', 'on');
        close all
        save(strcat(fullfile(saveDir,strcat(recDate,'-',mouseName,'-',sessionType,num2str(run))),'.mat'), 'mdatanorm', 'mdata', 'stddatanorm', 'LTMvMt', 'InstMvMt', 'Shift', 'time', 'fdata', 'hz','info');
