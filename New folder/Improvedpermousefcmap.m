...
qc=struct('Date',{'170821','170822'},...
          'MouseDay', {'a','a'},...
          'MouseNum', {'9510-5_P60','9511-2_P67'},...
          'Runs', {[1:6],[1:6],});  
%       qc=struct('Date',{'170821','170821','170821','170821','170822'},...
%           'MouseDay', {'a','a','a','a','a'},...
%           'MouseNum', {'9510-1_P60','9510-2_P60','9510-3_P60','9510-5_P60','9511-2_P67'},...
%           'Runs', {[1:6],[1:6],[1:6],[1:6],[1:6],}); 
...
mousenum=qc(i).MouseNum;
      path=['Z:\Rachel\Rachel_fcOIS\Development\'];
msk=['Z:\Rachel\Rachel_fcOIS\Development\' date '\' date '-' mousenum '_mask.tif'];      
...
switch catmode
    ...
    case 'onerun'
        ...
        oxy1=cell2mat(struct2cell(load([run label],'oxy')));
        deoxy1=cell2mat(struct2cell(load([run label],'deoxy')));
   ...
   if exist('oxyCat', 'var') % if/else added MDR 9/28
            oxyCat=cat(4,oxyCat,oxy1);
            deoxyCat=cat(4,deoxyCat,deoxy1);
            gcampCatall=cat(5,gcampCatall, gcamp6all);
        else
            oxyCat=oxy1;
            deoxyCat=deoxy1;
            gcampCatall=gcamp6all;
        end
...
oxy=squeeze(mean(oxyCat,4));
deoxy=squeeze(mean(deoxyCat,4));
...
gcamp3c=squeeze(mean(gcampCatall(:,:,1,:,:),5));
gcamp3u=squeeze(mean(gcampCatall(:,:,2,:,:),5));
...
%mkdir([path 'FC\']);
%q
print('-dpng', '-r600', '-painters', [path date '\' date '-' mousenum label '-MeanFC_MapsQC']);
 
save([path date '\' date '-' mousenum label '-MeanFC_DataQC'], 'Rs_Oxy', 'Rs_DeOxy','Rs_GCAMP3c', 'Rs_GCAMP3u', 'R_Data', 'seeds', 'WL', 'isbrain');
figure;imagesc(WL);
