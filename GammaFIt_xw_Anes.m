...
for excelRow = excelRows
    ...
    for n = runs
        ...
        for xInd = 1:size(HbT_filter,2)
            for yInd = 1:size(HbT_filter,1)
                if mask(yInd,xInd)
                    ...
                    fcn = @(param)he.fcn(param);
                    [~,pixHrfParam,obj_val,~,~] = evalc('fminsearchbnd(fcn,[1.4,1.2,0.05],[0.1,0.1,0.005],[4,6,1],options)');
        ...
        if exist(fullfile(saveDir,strcat(recDate,'-',mouseName,'-',sessionType,num2str(n),'_xw','.mat')),'file')
            save(fullfile(saveDir,strcat(recDate,'-',mouseName,'-',sessionType,num2str(n),'_xw','.mat')),...
        ...
        subplot(2,3,6)
        imagesc(objValues,[0 1000])
        axis image off
        colormap jet
        title('Objective Value')
        colorbar
        
        saveas(gcf,fullfile(saveDir,strcat(recDate,'-',mouseName,'-',sessionType,num2str(n),'_CalciumHbT_GammaFit_xw.png')));
        saveas(gcf,fullfile(saveDir,strcat(recDate,'-',mouseName,'-',sessionType,num2str(n),'_CalciumHbT_GammaFit_xw.fig')));  
...
for excelRow = excelRows
    ...
    mask = logical(mask_new.*xform_isbrain);
    for n = runs
      load(fullfile(saveDir,strcat(recDate,'-',mouseName,'-',sessionType,num2str(n),'_xw','.mat')),...
                'T','W','A','r','r2','hemoPred','objValues')
