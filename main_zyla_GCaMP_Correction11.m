...
%             clear raw_unregistered
%             load(fileName_cam2)
%             binnedRaw_cam2 = raw_unregistered(:,:,3,:);
%             clear raw_unregistered
%             disp(strcat('Register and Combine two cameras for ', rawName))
%
%             load(fullfile(saveDir,strcat(recDate,'-tform.mat')),'mytform')
%             rawdata = registerCam2andCombineTwoCams(binnedRaw_cam1,binnedRaw_cam2,mytform,[1,2],3);
%
...
%             load(fullfile(saveDir,processedName),'xform_datahb','systemInfo')
% 
%             disp('get FLuor data')
...
% 
% 
%             xform_fluorCorr = correctHb_differentBeta(xform_fluor,xform_datahb,[E_in(1) E_out(1)],[E_in(2) E_out(2)],dpIn,dpOut,1,1);
...
%             save(fullfile(saveDir, processedName_new),'xform_gcamp','xform_gcampCorr','xform_green','op_in', 'E_in','op_out', 'E_out','-v7.3')
%         end
%     end
% end
        ...
        if exist(fullfile(saveDir,processedName),'file')
            ...
            elseif strcmp(sessionType,'stim')
                ...
                disp('loading Non GRS data')
                    ...
                    load(fullfile(saveDir,strcat(recDate,'-',mouseName,'-stim',num2str(n),'_correction11.mat')),...
                ...
                texttitle_NoGSR = strcat(mouseName,'-stim',num2str(n)," ",'without GSR nor filtering, Correction11');
                output_NoGSR= fullfile(saveDir,strcat(recDate,'-',mouseName,'-stim',num2str(n),'-NoGSR_correction11'));
                ...
                close all
                save(fullfile(saveDir,strcat(recDate,'-',mouseName,'-stim',num2str(n),'_correction11.mat')),'goodBlocks_NoGSR','ROI_NoGSR','-append')
