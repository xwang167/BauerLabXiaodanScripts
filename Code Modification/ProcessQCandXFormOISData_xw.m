...
for runInd = 1:runNum
    ...
    if exist(runInfo.saveHbFile,'file') %checks to see if the raw data were already processed
        ...
        [op, E] = getHbOpticalProperties_new(runInfo.lightSourceFiles{runInfo.hbChInd});%%%%musp to the function
        ...
        data = nan(size(xform_datahb,1),size(xform_datahb,2),size(oxy,4),3);
        if ~isempty(runInfo.fluorChInd) % if there are fluor channels
            contrastName(end+1) = {'Fluor'};
            data = nan(size(xform_datahb,1),size(xform_datahb,2),size(oxy,4),4);
        end
        Fs = runInfo.SamplingRate;
        %GSR
        data(:,:,:,1) = mouse.process.gsr(squeeze(xform_datahb(:,:,1,:)),xform_isbrain); % gsr
        data(:,:,:,2) = mouse.process.gsr(squeeze(xform_datahb(:,:,2,:)),xform_isbrain);
        data(:,:,:,3) =  mouse.process.gsr(squeeze(xform_datahb(:,:,1,:)+xform_datahb(:,:,2,:)),xform_isbrain);
        if ~isempty(runInfo.fluorChInd)
            data(:,:,:,4) = mouse.process.gsr(xform_datafluor,xform_isbrain);
        end
        ...
        [fh,R,Rs,refseeds,seedradpix,bilatFC] = qcFC(data,fRange,xform_isbrain,contrastName,runInfo);
        
        saveSeedFCName = [runInfo.saveFilePrefix '-seedFC-' fStr];
        save(saveSeedFCName,'contrastName','R','Rs','bilatFC','seedradpix','refseeds','-v7.3');
        for ii = 1:size(data,4)
            saveFCQCFigName = [runInfo.saveFCQCFig '-' contrastName{ii}];
            saveas(fcFig(ii),[saveFCQCFigName '.png']);
            close(fcFig(ii));
        end
        
        fRange = [0.4 4]; %changed 4/29 from [0.009 0.08] in order to cover delta band instead
        fStr = [num2str(fRange(1)) '-' num2str(fRange(2))];
        fStr(strfind(fStr,'.')) = 'p';
        
        [fh,R,Rs,refseeds,seedradpix,bilatFC] = qcFC(data,fRange,Fs,xform_isbrain,contrastName,runInfo);
