...
%Pre-FC Process:
    ...
    %Initialize
        ...
        %Shape Data
        data_full(:,:,:,1)=squeeze(xform_datahb(:,:,1,:)); %pixel x pixel x time x species
        ...
        if ~isempty(runInfo.fluorChInd)
            ...
            clear  xform_datafluorCorr xform_datahb xform_datafluor
    %Filter
        ...
        data_delta(:,:,:,ii)=filterData(data_full(:,:,:,ii),0.4,4,runInfo.samplingRate); 
            ...
            clear data_full
    %GSR
        parfor  i=1:size(data_isa,4)
        data_isa(:,:,:,i) = gsr(squeeze(data_isa(:,:,:,i)),xform_isbrain); 
        data_delta(:,:,:,i) = gsr(squeeze(data_delta(:,:,:,i)),xform_isbrain);
        end
...
%QC FC
    ...
    %Saving   
            ...
            %Add frequency range!
                for contrastInd = 1:size(data_isa,4)
    ...
    %Saving   
            ...
            %Add frequency range!
                for contrastInd = 1:size(data_delta,4)
                saveFCQCFigName = [runInfo.saveFCQCFig '-' contrastName{contrastInd} ];
                saveas(fh(contrastInd),[saveFCQCFigName '.png']);
                close(fh(contrastInd));
                end
